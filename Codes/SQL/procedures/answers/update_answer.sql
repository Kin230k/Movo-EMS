...
DECLARE
    ...
    v_exists INT;
BEGIN
    CALL check_user_permission(p_current_user_id, 'update_answer');
    ...
    IF p_answer_type IS NOT NULL THEN
        ...
        IF v_type NOT IN ('text','rating','numeric','options') THEN
            RAISE EXCEPTION 'invalid answerType: %, allowed: text, rating, numeric, options', v_type;
        END IF;
    ELSE
    ...
    -- Validate rating
    IF v_type = 'rating' AND (p_rating < 1 OR p_rating > 5) THEN
    ...
    -- Update main answer
    UPDATE ANSWERS
    SET submissionId = p_submission_id,
        questionId   = p_question_id,
        answeredAt   = COALESCE(p_answered_at, now())
    WHERE answerId = p_answer_id;

    -- Delete existing subtypes
    DELETE FROM TEXT_ANSWERS    WHERE answerId = p_answer_id;
    DELETE FROM RATING_ANSWERS  WHERE answerId = p_answer_id;
    DELETE FROM NUMERIC_ANSWERS WHERE answerId = p_answer_id;
    DELETE FROM ANSWER_OPTIONS  WHERE answerId = p_answer_id;
