...
export interface GetQuestionAnswersBySubmissionIdData {
  submissionId: string;
  projectId: string;
}

export interface GetQuestionAnswersBySubmissionIdResult {
  success: boolean;
  data?: any | null;
  issues?: FieldIssue[];
}

export async function getQuestionAnswersBySubmissionIdHandler(
  request: CallableRequest<GetQuestionAnswersBySubmissionIdData>
): Promise<GetQuestionAnswersBySubmissionIdResult> {
  ...
  try {
    ...
    const answers = await AnswerService.getQuestionAnswersBySubmissionId(submissionId);
