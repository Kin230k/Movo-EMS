...
export async function createQuestionsHandler(
  ...
  if (!data) {
    return {
      success: false,
      issues: [{ field: 'data', message: 'Request data is missing' }],
    };
  }
    ...
    questions.forEach((q, idx) => {
      ...
      if (!isNonEmptyString(q.questionText)) {
        issues.push({
          ...
          message: 'questionText must be a non-empty string',
        });
      }
      ...
      if (q.typeCode === 'MULTIPLE_CHOICE') {
        if (!Array.isArray(q.options) || q.options.length === 0) {
          issues.push({
            ...
            message: 'Options are required for OPTION type questions',
          });
        } else {
        ...
        if (!Array.isArray(q.criteria)) {
          ...
          q.criteria.forEach((c, cidx) => {
            ...
            if (!['PASS', 'FAIL'].includes(c.effect)) {
              issues.push({
                field: `questions[${idx}].criteria[${cidx}].effect`,
                message: 'effect must be either "PASS" or "FAIL"',
              });
            }
          });
        }
