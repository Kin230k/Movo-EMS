...
export class EmailMapper extends BaseMapper<Email> {
  async save(entity: Email): Promise<void> {
    ...
    if (op === Operation.UPDATE) {
      ...
      await pool.query('CALL update_email($1, $2, $3, $4)', [
        emailId,
        title,
        body,
        formId,
      ...
      await pool.query('CALL create_email($1, $2, $3)', [title, body, formId]);
    }
  }
  ...
  async getAll(): Promise<Email[]> {
    const result = await pool.query('SELECT * FROM get_all_emails()');
    return result.rows.map(
      (row) => new Email(row.title, row.body, row.formId, row.emailId)
    );
  }

  async delete(id: string): Promise<void> {
    await pool.query('CALL delete_email($1)', [id]);
  }
}
