...
export class DecisionRuleMapper extends BaseMapper<DecisionRule> {
  async save(entity: DecisionRule): Promise<void> {
    ...
    if (op === Operation.UPDATE) {
      ...
      await pool.query('CALL create_decision_rule($1, $2, $3, $4, $5, $6)', [
        name,
        description,
        formId,
        priority,
  ...
  async getAll(): Promise<DecisionRule[]> {
    const result = await pool.query('SELECT * FROM get_all_decision_rules()');
  ...
  async delete(id: string): Promise<void> {
    await pool.query('CALL delete_decision_rule($1)', [id]);
  }
}
