...
export class RuleCriterionMapper extends BaseMapper<RuleCriterion> {
  async save(entity: RuleCriterion): Promise<void> {
    ...
    if (op === Operation.UPDATE) {
      if (!ruleCriterionId)
        throw new Error('RuleCriterion ID required for update');
      await pool.query('CALL update_rule_criterion($1, $2, $3, $4)', [
        ruleCriterionId,
        ruleId,
        criterionId,
        required,
      ...
      await pool.query('CALL create_rule_criterion($1, $2, $3)', [
        ruleId,
        criterionId,
        required,
      ]);
  ...
  async getAll(): Promise<RuleCriterion[]> {
    const result = await pool.query('SELECT * FROM get_all_rule_criterions()');
  ...
  async delete(id: string): Promise<void> {
    await pool.query('CALL delete_rule_criterion($1)', [id]);
  }
}
