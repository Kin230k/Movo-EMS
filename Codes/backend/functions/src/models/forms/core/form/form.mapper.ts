...
export class FormMapper extends BaseMapper<Form> {
  ...
  async save(entity: Form): Promise<void> {
    ...
    if (op === Operation.UPDATE) {
      ...
      await this.pool.query(
        'CALL update_form($1, $2, $3)',
        [formId, projectId, locationId]
      );
    } else {
      await this.pool.query(
        'CALL create_form($1, $2)',
  ...
  async getById(id: string): Promise<Form | null> {
    ...
    return result.rows.length ? new Form(
  ...
  async getAll(): Promise<Form[]> {
    const result = await this.pool.query('SELECT * FROM get_all_forms()');
    return result.rows.map(row => new Form(
      row.projectId,
      row.locationId,
      row.formId
    ));
  ...
  async delete(id: string): Promise<void> {
    await this.pool.query('CALL delete_form($1)', [id]);
  }
}
