...
export class OptionMapper extends BaseMapper<Option> {
  async save(entity: Option): Promise<void> {
    ...
    const { optionId, optionText, questionId, isCorrect, displayOrder } = entity;

    if (!optionText) throw new Error('Option text is required');
    if (!questionId) throw new Error('Question ID is required');
    if (isCorrect === undefined || isCorrect === null) throw new Error('isCorrect flag is required');
    if (displayOrder === undefined || displayOrder === null) throw new Error('Display order is required');

    if (op === Operation.UPDATE) {
      if (!optionId) throw new Error('Option ID is required for update');
      await pool.query(
        'CALL update_option($1, $2, $3, $4, $5, $6)',
        [currentUserId, optionId, optionText, questionId, isCorrect, displayOrder]
      );
    } else {
      await pool.query(
        'CALL create_option($1, $2, $3, $4, $5)',
        [currentUserId, optionText, questionId, isCorrect, displayOrder]
      );
    }
  }
  ...
  async getAll(): Promise<Option[]> {
    ...
    const result = await pool.query('SELECT * FROM get_all_options($1)', [currentUserId]);
