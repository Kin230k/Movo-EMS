...
export class OptionMapper extends BaseMapper<Option> {
  ...
  async save(entity: Option): Promise<void> {
    ...
    if (op === Operation.UPDATE) {
      ...
      await this.pool.query(
        'CALL update_option($1, $2, $3, $4, $5)',
        [optionId, optionText, questionId, isCorrect, displayOrder]
      );
    } else {
      await this.pool.query(
        'CALL create_option($1, $2, $3, $4)',
        [optionText, questionId, isCorrect, displayOrder]
      );
    }
  }
  ...
  async getById(id: string): Promise<Option | null> {
    ...
    return result.rows.length ? new Option(
  ...
  async getAll(): Promise<Option[]> {
    const result = await this.pool.query('SELECT * FROM get_all_options()');
    return result.rows.map(row => new Option(
      row.optionText,
      row.questionId,
      row.isCorrect,
      row.displayOrder,
  ...
  async delete(id: string): Promise<void> {
    await this.pool.query('CALL delete_option($1)', [id]);
  }
}
