...
export class QuestionTypeMapper extends BaseMapper<QuestionType> {
  ...
  async save(entity: QuestionType): Promise<void> {
    ...
    if (op === Operation.UPDATE) {
      ...
      await this.pool.query(
        'CALL update_question_type($1, $2)',
        [typeCode, description]
      );
    } else {
      await this.pool.query(
        'CALL create_question_type($1)',
        [description]
      );
    }
  }
  ...
  async getById(id: string): Promise<QuestionType | null> {
    ...
    return result.rows.length ? new QuestionType(
  ...
  async getAll(): Promise<QuestionType[]> {
    const result = await this.pool.query('SELECT * FROM get_all_question_types()');
    return result.rows.map(row => new QuestionType(
      row.description,
      row.typeCode
    ));
  }
  ...
  async delete(id: string): Promise<void> {
    await this.pool.query('CALL delete_question_type($1)', [id]);
  }
}
