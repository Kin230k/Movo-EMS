...
export class QuestionMapper extends BaseMapper<Question> {
  async save(entity: Question): Promise<void> {
    ...
    if (op === Operation.UPDATE) {
      ...
      await pool.query('CALL update_question($1, $2, $3, $4, $5)', [
        questionId,
        typeCode,
        questionText,
        formId,
      ...
      await pool.query('CALL create_question($1, $2, $3, $4)', [
        typeCode,
        questionText,
        formId,
        interviewId,
  ...
  async getAll(): Promise<Question[]> {
    const result = await pool.query('SELECT * FROM get_all_questions()');
  ...
  async delete(id: string): Promise<void> {
    await pool.query('CALL delete_question($1)', [id]);
  }
}
