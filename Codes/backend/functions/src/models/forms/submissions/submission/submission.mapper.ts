...
export class SubmissionMapper extends BaseMapper<Submission> {
  async save(entity: Submission): Promise<void> {
    ...
    if (op === Operation.UPDATE) {
      ...
      await pool.query('CALL update_submission($1, $2, $3, $4, $5, $6, $7)', [
        submissionId,
        formId,
        userId,
        interviewId,
      ...
      await pool.query('CALL create_submission($1, $2, $3, $4, $5, $6)', [
        formId,
        userId,
        interviewId,
        dateSubmitted,
  ...
  async getAll(): Promise<Submission[]> {
    const result = await pool.query('SELECT * FROM get_all_submissions()');
  ...
  async delete(id: string): Promise<void> {
    await pool.query('CALL delete_submission($1)', [id]);
  }
}
