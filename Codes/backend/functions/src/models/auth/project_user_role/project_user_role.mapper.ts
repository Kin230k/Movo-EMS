...
export class ProjectUserRoleMapper extends BaseMapper<ProjectUserRole> {
  async save(entity: ProjectUserRole): Promise<void> {
    ...
    if (op === Operation.UPDATE) {
      ...
      await pool.query('CALL update_project_user_role($1, $2, $3, $4)', [
        projectUserRoleId,
        userId,
        projectId,
        roleId,
      ...
      await pool.query('CALL create_project_user_role($1, $2, $3)', [
        userId,
        projectId,
        roleId,
      ]);
  ...
  async getAll(): Promise<ProjectUserRole[]> {
    const result = await pool.query(
      'SELECT * FROM get_all_project_user_roles()'
    );
    return result.rows.map(this.mapRowToEntity);
  }

  async delete(id: string): Promise<void> {
    await pool.query('CALL delete_project_user_role($1)', [id]);
  }
