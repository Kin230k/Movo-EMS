...
export class AdminsRoleMapper extends BaseMapper<AdminsRole> {
  async save(entity: AdminsRole): Promise<void> {
    ...
    if (op === Operation.UPDATE) {
      ...
      await pool.query('CALL update_admins_role($1, $2, $3, $4)', [
        currentUserId,
        adminRoleId,
        adminId,
        roleId,
      ...
      await pool.query('CALL create_admins_role($1, $2, $3)', [currentUserId, adminId, roleId]);
    }
  }
  ...
  async getById(id: string): Promise<AdminsRole | null> {
    ...
    const result: QueryResult = await pool.query(
      'SELECT * FROM get_admins_role_by_id($1, $2)',
      [currentUserId, id]
    );
  ...
  async getAll(): Promise<AdminsRole[]> {
    ...
    const result = await pool.query('SELECT * FROM get_all_admins_roles($1)', [currentUserId]);
  ...
  async delete(id: string): Promise<void> {
    ...
    await pool.query('CALL delete_admins_role($1, $2)', [currentUserId, id]);
  }
