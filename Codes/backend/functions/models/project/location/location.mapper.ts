...
export class LocationMapper extends BaseMapper<Location> {
  ...
  async save(entity: Location): Promise<void> {
    ...
    if (op === Operation.UPDATE) {
      if (!entity.locationId) throw new Error('Location ID is required for update');
      await this.pool.query(
        'CALL update_location($1, $2, $3, $4, $5, $6)',
        [locationId, name, projectId, siteMap, longitude, latitude]
      );
    } else {
      await this.pool.query(
        'CALL create_location($1, $2, $3, $4, $5)',
  ...
  async getAll(): Promise<Location[]> {
    const result = await this.pool.query('SELECT * FROM get_all_locations()');
